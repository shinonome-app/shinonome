  ...
  describe '.execute' do
    ...
    context '正しい引数を与えた場合' do
      it 'work_workerを含むResultを返す' do
        command = Shinonome::ExecCommand::Command.new(['耕作員追加', work.id, worker.id, '校正者'])
    ...
    context 'worker_idが数値ではない場合' do
      it '例外をあげる' do
        command = Shinonome::ExecCommand::Command.new(['耕作員追加', work.id, 'abc', '校正者'])
    ...
    context 'work_idが存在しない場合' do
      it '例外をあげる' do
        command = Shinonome::ExecCommand::Command.new(['耕作員追加', 100000, 'abc', '校正者'])
    ...
    context '存在しないworker_roleを与えた場合' do
      it '例外をあげる' do
        command = Shinonome::ExecCommand::Command.new(['耕作員追加', work.id, worker.id, '役割テスト'])
