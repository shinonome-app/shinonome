...
RSpec.describe Shinonome::ExecCommand::Command::GetPerson do
  describe '.execute' do
    ...
    it '正しいCSVが生成される' do
      command = Shinonome::ExecCommand::Command.new(['person'])

      Dir.mktmpdir do |dir|
        Shinonome::ExecCommand::Command::GetPerson.new.execute(command, output_dir: dir)
        ...
        File.open(output_file) do |f|
          ...
          expect(row2[14]).to eq person2.person_secret.memo
          ...
          expect(row3[14]).to eq person3.person_secret.memo
