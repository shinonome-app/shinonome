  ...
  describe '.execute' do
    ...
    context '正しい引数を与えた場合' do
      it 'workを含むResultを返す' do
        result = Shinonome::ExecCommand::Command::AddWork.new.execute(**args)
    ...
    context 'sortkeyがない場合' do
      it 'sortkeyを生成したworkを含むResultを返す' do
        ...
        result = Shinonome::ExecCommand::Command::AddWork.new.execute(**args1)
        expect(result).to be_successful
        command = result.command_result
        expect(command.title).to eq 'タイトル1'
        expect(command.title_kana).to eq 'たいとる1'
    ...
    context '著作権フラグの値が正しくない場合' do
      it '例外をあげる' do
        ...
        expect do
          Shinonome::ExecCommand::Command::AddWork.new.execute(**args2)
        end.to raise_error(
    ...
    context '文字遣い種別の値が正しくない場合' do
      it '例外をあげる' do
        ...
        expect do
          Shinonome::ExecCommand::Command::AddWork.new.execute(**args3)
        end.to raise_error(
          Shinonome::ExecCommand::FormatError,
    ...
    context '状態の値が正しくない場合' do
      it '例外をあげる' do
        ...
        expect do
          Shinonome::ExecCommand::Command::AddWork.new.execute(**args4)
        end.to raise_error(
          Shinonome::ExecCommand::FormatError,
